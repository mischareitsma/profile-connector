...
export function lvFormat(messageString: string): string {
	const lvArray = [];
	...
	return (returnLvFormat + messageString);
}
	...
	while (bytePointer < messageLength) {
		...
		if (extractChar === 0) {
			...
			for (let loopFor = numberOfBufferedLine - 1; loopFor >= 0; loopFor--) {
				extractChar = (
					messageString.readUInt8(bytePointer + loopFor) *
					byteCalcNumber
				) + extractChar;
		...
		returnString.push(messageString.slice(
			bytePointer + numberOfBufferedLine, bytePointer + extractChar
		));
...
export function parseResponse(
	...
	// unpacking multiple times to get the token, remove the endiness by extracting
	// from position 2
	...
	if (serviceClass === 5) {
		returnString = (
			returnArray[2].toString(encoding) +
			String.fromCharCode(0) +
			returnArray[3].toString(encoding)
		);
	}
	return returnString;
}
...
export function connectionObject(envUser: string, envPassword: string): string {
	...
	return v2lvFormat(prepareString);
}
...
export function getObjectType(fileName: string): FileDetails {
	const elementBaseName: string = path.basename(fileName);
	let elementName = elementBaseName.substr(0, elementBaseName.lastIndexOf("."));
	const elementExtension = elementBaseName.substr(
		elementBaseName.lastIndexOf(".") + 1,
		elementBaseName.length
	);
...
export function pack(totalLength: number): string {
	...
	const quotient = Math.floor(totalLength / 256);
	const firstByte = String.fromCharCode(quotient);
	const secondByte = String.fromCharCode(totalLength - (quotient * 256));
